...
urlpatterns = [
    re_path(r'^$', vip_request_resource.handle_request,
        name='vip.insert'),
    re_path(r'^all/$', vip_list_all_resource.handle_request,
        name='vip.list_all'),
    re_path(r'^ipv4/all/$', vip_list_all_ipv4_resource.handle_request,
        name='vip.ipv4.all'),
    re_path(r'^ipv6/all/$', vip_list_all_ipv6_resource.handle_request,
        name='vip.ipv6.all'),
    re_path(r'^create/$', vip_create_resource.handle_request,
        name='vip.create'),
    re_path(r'^(?P<id_vip>\d+)/criar/$', vip_create_resource.handle_request,
        name='vip.create.by.pk'),
    re_path(r'^remove/$', vip_remove_resource.handle_request,
        name='vip.remove'),
    re_path(r'^real/$', vip_real.handle_request,
        name='vip.real'),
    # re_path(r'^(?P<id_vip>[^/]+)/filter/$', vip_l7_resource.handle_request,
    #     name='vip.get.l7.by.pk'),
    re_path(r'^(?P<id_vip>[^/]+)/$', vip_request_resource.handle_request,
        name='vip.get.update.by.pk'),
    re_path(r'^(?P<id_vip>[^/]+)/(?P<operacao>maxcon)/(?P<maxcon>[^/]+)/$', vip_maxcon.handle_request,
        name='vip.update.maxcon.by.pk'),
    re_path(r'^(?P<id_vip>[^/]+)/(?P<operacao>healthcheck)/$', vip_healthcheck_resource.handle_request,
        name='vip.update.healthcheck.by.pk'),
    re_path(r'^(?P<id_vip>[^/]+)/(?P<operacao>persistence)/$', vip_persistence_resource.handle_request,
        name='vip.update.persistence.by.pk'),
    re_path(r'^(?P<id_vip>[^/]+)/(?P<operacao>priority)/$', vip_priority.handle_request,
        name='vip.update.priority.by.pk'),
    re_path(r'^delete/(?P<id_vip>[^/]+)/$', vip_delete_resource.handle_request,
        name='vip.delete.by.pk'),
    re_path(r'^validate/(?P<id_vip>[^/]+)/$', vip_validate_resource.handle_request,
        name='vip.validate.by.pk'),
    re_path(r'^real/edit/$', vip_real_edit.handle_request,
        name='vip.real.edit'),
    # re_path(r'^real/valid/$', vip_real_valid.handle_request,
    #     name='vip.real.valid'),
    # re_path(r'^l7/(?P<id_vip>[^/]+)/$', vip_l7_resource.handle_request,
    #     name='vip.get.l7.by.pk'),
    # re_path(r'^l7/(?P<id_vip>[^/]+)/validate/$', vip_l7_validate_resource.handle_request,
    #     name='vip.l7.validate.by.pk'),
    # re_path(r'^l7/(?P<id_vip>[^/]+)/apply/$',vip_l7_apply_resource.handle_request,
    #      name='vip.l7.apply.by.pk'),
    # re_path(r'^l7/(?P<id_vip>[^/]+)/rollback/$', vip_l7_rollback_resource.handle_request,
    #     name='vip.l7.rollback.by.pk'),
    re_path(r'^add_block/(?P<id_vip>\d+)/(?P<id_block>\d+)/(?P<override>\d+)[/]?$', vip_add_block_resource.handle_request,
        name='vip.add.block'),
    # re_path(r'^dsrl3/all/$', drsl3_all.handle_request,
    #     name='dsrl3.vip.all'),
    # re_path(r'^dsrl3/(?P<id_dsrl3_vip>[^/]+)/$', dsrl3tovip.handle_request,
